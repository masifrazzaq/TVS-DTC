...
public class RunThermalCameras {
	static {
	...
	public static void main(String[] args) throws Exception {
		RunThermalCameras t = new RunThermalCameras();
		t.initialize();
		t.playVideo();
	}
	...
	public void playVideo() throws Exception{	
		...
		while (true) {
			...
			System.loadLibrary( Core.NATIVE_LIBRARY_NAME );
		      ...
		      SensorRtFrame rtFrameK=new SensorRtFrame("URL");
		  
		      long t0=System.currentTimeMillis();
		        long maxT=300L; //3000000L

		while(System.currentTimeMillis()-t0<maxT) {
			 Mat m0=rtFrameK.getFrame().toMat();
			...
			Imgproc.resize(m0, m0, new Size(CONFIG.FRAME_WIDTH, CONFIG.FRAME_HEIGHT),
...
//				Functions.processFrame(camera, inFrame, diffFrame, mBGSub);   //Asif
				...
				for (int k = 0; k < tracker.tracks.size(); k++) {
					...
					if (traceNum > 1) {
						for (int jt = 1; jt < tracker.tracks.get(k).trace
								.size(); jt++) {
// this gives the line to the moving object.
							Imgproc.line(imag,
